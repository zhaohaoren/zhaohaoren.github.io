...
var getContent2023 = () => {
  // 内容渲染
  let ul = $(".micro-blog .ul-content-2023");
  ul.append(`
  ...
  let process = (res) => {
    ...
    res.map((i) => {
      ...
      item += `<a href="#2023-${id}" name=2023-${id}>#${id}</a>`;
      item += `</div>`;
  ...
  let reqUrlWithProcess = () => {
    let url = "/micro-blog/2023.json";
    $.ajax({
      url: url,
      success: (res) => {
        process(res);
        localStorage.setItem("micro-blog-2023", JSON.stringify(res));
      },
      error: (jqXHR, textStatus, errorThrown) => {
        let res = localStorage.getItem("micro-blog-2023");
        if (res == null) {
